			...
			<view class="header">
				<view class="title">
					<view class="HeadPortrait" type="primary" :loading="loading" :disabled="disabled" @click="upload('header_photo')">
						<image :src="DealerInfoList.avatar" v-if='Boolean(DealerInfoList.avatar)' />
					</view>
					<view class="HeadInfo">
						<view class="name">{{DealerInfoList.shop_name}}</view>
						<view class="num">电话号码：{{DealerInfoList.phone}}</view>
					</view>
				</view>
			</view>
			<view class="main">
...
<script>
	...
	export default {
		...
		methods: {
			//发起请求(我的信息)
			getInfo(){
				this.$api.MyDealerInfo({}).then((res) => {
					this.DealerInfoList = res.data.data;
					this.dealer_phone = res.data.data.phone;
				}).catch((err) => {
